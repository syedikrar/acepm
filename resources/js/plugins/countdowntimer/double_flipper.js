class DoubleFlipperCountdown {
    ...
    constructor(userOptions) {
        ...
        this.elementClassPrefix = 'countDownDouble_';
        this.interval = null;
        this.digitConts = {};
            ...
            if (data.hasOwnProperty(intervalName)) {
                data[intervalName].forEach((digit, index) => {
                    if (this.lastTick !== null && this.lastTick[intervalName][index] !== data[intervalName][index]) {
                        try {
                            this._getDigitCont(intervalName, 0).innerHTML =
                                this._getDigitElementString(data[intervalName][0], this.lastTick[intervalName][0]);
                            this._getDigitCont(intervalName, 1).innerHTML =
                                this._getDigitElementString(data[intervalName][1], this.lastTick[intervalName][1]);
                        } catch(e) {}
                    }
                });
            }
