...
axios.interceptors.request.use(config => {
    ...
    const shop = store.getters['auth/shop']
    if (shop) {
...
axios.interceptors.response.use(response => {
    ...
    error.response.data.message !== undefined && app.showSnack(error.response.data.message || 'Something went wrong.')
    error.response.data.error !== undefined && app.showSnack(error.response.data.error || 'Error occurred.')
    return Promise.reject(error)
})
